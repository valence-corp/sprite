  ...
  add = {
    ...
    text: {
      ...
      note: (markdown: string) => {
        this.append(`<p class="note">${markdown}</p>`);
      },
    },
    list: {
...
class JuniperMarkDownMethodNode extends JuniperMarkDownNode {
  ...
  constructor(
    ...
    this.markdown.add.heading.h3(`${this.parentString}.${node.name}`);
  ...
  buildPath = (): IJuniperNodePath => {
    ...
    const relative = path.join("/classes/", this.parentNode.name);
    return {
      dir,
      relative,
  ...
  processBlockTags = (blockTags: any[]) => {
    blockTags.forEach((tag: any) => {
      ...
      if (tag.tag === "@note") {
        this.markdown.add.heading.h4("Note");
...
class JuniperClassNode {
  ...
  processNodes = () => {
    this.node.children.forEach((child) => {
      // constructors
      if (child.kind === 512) {
        this.parseConstructor(child, this.node);
      }
      // // properties
      // if (node.kind === 1024) {
